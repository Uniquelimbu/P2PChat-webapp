...
export function MessageArea() {
  ...
  return (
    <div className="flex-1 flex flex-col bg-background h-full relative overflow-hidden">
      ...
      {/* Header */}
      <div className="h-14 border-b border-border flex items-center justify-between px-5 bg-card/20 backdrop-blur-sm flex-shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          {activeDm ? (
            <div className="flex items-center gap-2 min-w-0">
              <div className="w-7 h-7 rounded-md bg-accent/15 border border-accent/30 flex items-center justify-center font-mono font-bold text-xs text-accent flex-shrink-0">
                {activeDm.name.charAt(0).toUpperCase()}
              </div>
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-accent truncate">{activeDm.name}</span>
                  <span className="text-[9px] bg-accent/10 text-accent/70 border border-accent/20 px-1.5 py-0.5 rounded font-mono whitespace-nowrap hidden sm:flex items-center gap-1 flex-shrink-0">
                    <Lock className="w-2.5 h-2.5" /> PRIVATE
                  </span>
                </div>
                <div className="text-[10px] text-muted-foreground/50 font-mono">{activeDm.ip}</div>
              </div>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <span className="text-primary font-bold text-xl">#</span>
              <span className="font-semibold text-base truncate">{activeRoom!.name}</span>
            </div>
          )}
        ...
        <Button
          ...
          className={cn(
            "font-mono text-[11px] gap-1.5 h-8 px-3 flex-shrink-0 transition-colors",
            isInspectorOpen
      ...
      <div
        ...
        className="flex-1 overflow-y-auto px-5 py-4 flex flex-col scrollbar-custom relative"
      >
        {!activeDm && isFetching && roomMessages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center font-mono text-primary/60 text-sm animate-pulse">
            LOADING_ARCHIVE...
          </div>
        ) : items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-muted-foreground/40 font-mono text-sm">
            {activeDm ? (
              <>
                <Lock className="w-8 h-8 opacity-20" />
                <span>SECURE CHANNEL OPEN</span>
                <span className="text-xs opacity-60">Messages go directly to {activeDm.name}</span>
              </>
            ) : (
              <>
          ...
          <div className="flex flex-col gap-0.5">
            {items.map((item) => {
              ...
              return (
                <div
                  key={item.key}
                  className={cn("flex gap-2.5", isMe ? "flex-row-reverse" : "flex-row", isConsecutive ? "mt-0.5" : "mt-3")}
                >
                  ...
                  <div className={cn("flex flex-col max-w-[75%]", isMe ? "items-end" : "items-start")}>
                    {!isConsecutive && (
                      <div className={cn("flex items-baseline gap-2 mb-1", isMe ? "flex-row-reverse" : "flex-row")}>
                        <span className={cn("text-xs font-semibold", isMe ? "text-accent" : "text-primary")}>
                          {senderName}
                        </span>
                        ...
                        {!isMe && senderIp && (
                          <span className="text-[9px] text-muted-foreground/30 font-mono hidden md:block">
                            [{senderIp}]
                          </span>
                        )}
                      </div>
      ...
      {showScrollBtn && (
        <button
          ...
          className="absolute bottom-20 right-5 z-20 w-8 h-8 rounded-full bg-card border border-border shadow-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary/50 transition-all"
        >
      ...
      {/* DM error banner */}
      {dmError && (
        <div className="mx-5 mb-0 mt-2 flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/30 text-destructive text-xs font-mono">
      ...
      {/* Input */}
      <div className="px-5 py-3 border-t border-border bg-background/50 backdrop-blur-sm flex-shrink-0">
        <form onSubmit={handleSend} className="flex items-center gap-2 max-w-4xl mx-auto">
          <div className="relative flex-1">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-primary font-mono font-bold pointer-events-none select-none">
              &gt;
            </span>
            <Input
              value={content}
              ...
              className="pl-7 pr-3 h-11 rounded-xl bg-card/60 border-border focus-visible:border-primary/50 focus-visible:ring-1 focus-visible:ring-primary/30 text-sm"
          ...
          <Button
            type="submit"
            size="sm"
            className="h-11 px-5 rounded-xl font-mono text-xs gap-2 flex-shrink-0"
